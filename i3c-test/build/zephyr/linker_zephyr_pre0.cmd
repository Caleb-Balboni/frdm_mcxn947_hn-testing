...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    SRAMX (NOLOAD) : { __SRAMX_start = .; KEEP(*(SRAMX)) KEEP(*(SRAMX.*)) __SRAMX_end = .; } > SRAMX __SRAMX_size = __SRAMX_end - __SRAMX_start; __SRAMX_load_start = LOADADDR(SRAMX);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
